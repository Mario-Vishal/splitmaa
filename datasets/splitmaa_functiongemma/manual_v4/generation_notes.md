...
## Current Status
- `train.jsonl`, `validation.jsonl`, and `test.jsonl` contain 1,500 manually authored rows:
  - train: 1,050
  - validation: 225
  - test: 225
- Strict routing validation passes for all 1,500 rows.
...
- Input uniqueness checks report zero exact duplicates. The latest near-duplicate scan reports 12 low-risk pairs at threshold 0.78, mostly short command shapes that remain semantically distinct.
- The initial 600-row manual seed target is complete, and expansion has reached a 1,500-row checkpoint.
- Passing train and validation splits have been converted to FunctionGemma format:
  - `train.functiongemma.jsonl`: 1,050 examples
  - `validation.functiongemma.jsonl`: 225 examples
